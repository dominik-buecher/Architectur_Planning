...
grid_window = GridWindow(root, rows, cols, num_cows, device)
...
if train is True:
    ...
    for episode in range(max_episodes):
        state = grid_window.get_state().to(dqn_agent.device)  # Move state to GPU
        total_reward = 0
        done = False
        ...
        while not done:
            ...
            next_state = grid_window.get_state().to(dqn_agent.device)  # Move state to GPU
            reward = grid_window.get_reward(state, next_row, next_col)
            done = True if reward == 500 or action_counter >= 1000 else False

            # Move data to GPU without converting to numpy
