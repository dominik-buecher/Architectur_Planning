...
#     def forward(self, x):
#         # x is an array of mower-position and cow-positions
#         # Step1 calculate Self and Cow Position coordinates
#         global n_rows
#         global n_cols
    ...
    def run(self):
        ...
        def epsilon_greedy_action(state, epsilon, guided_epsilon_probability=0.0):
            if random.uniform(0, 1) < epsilon:
                ...
                state_tensor = torch.tensor([state] + [get_state_from_coord(c[0], c[1]) for c in g.get_cow_positions()], dtype=torch.float32)  # Wrap the state in a tensor
                # print(state_tensor)
        ...
        for episode_idx in range(max_episodes):
            ...
            while not done:
                ...
                if len(replay_memory) > batch_size:
                    ...
                    next_state_batch = torch.tensor([[t.next_state_idx] + (t.state_indexes_cows)  for t in transitions], dtype=torch.float32)
