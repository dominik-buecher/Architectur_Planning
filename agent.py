...
def state_to_tensor(cells, mower_position):
    flattened_cells = [cell["bg"] for row in cells for cell in row]
    flattened_cells[mower_position] = "#006400"  # Markiere die Position des Rasenmähers
    return torch.tensor(flattened_cells, dtype=torch.float32)

def main():

    env = gym.make('DeineUmgebungHier')  # Du musst hier die Umgebung anpassen
    input_size = 10  # Anzahl der Merkmale im Zustand
    output_size = env.action_space.n  # Anzahl der Aktionen
    
    env = gym.make('CartPole-v1')  # Du musst hier die Umgebung anpassen oder deine eigene erstellen
    input_size = env.observation_space.shape[0]  # Anzahl der Zustände
    output_size = env.action_space.n  # Anzahl der Aktionen
    ...
    for episode in range(NUM_EPISODES):
        ...
        while True:
            ...
            tensor_state = torch.tensor(state, dtype=torch.float32).to(agent.device)
            action = agent.select_action(tensor_state)
            next_state, reward, done, _ = env.step(action)

            tensor_next_state = torch.tensor(next_state, dtype=torch.float32).to(agent.device)
